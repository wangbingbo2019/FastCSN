        ...
        for k in cell_id:
            if gene1[k] * gene2[k] > 0:
                ...
                while (math.sqrt(pow(d2_0 - d2_1, 2) + pow(d1_0 - d1_1, 2)) < pow(10, -5)) & count < maxiter:
                    ...
                    count = count + 1
                if count >= 10000:
                    print(f"Iteration at cell {k} exceeds {maxiter}")
                    return
    ...
    for cl in range(0, K):
        ...
        for i in range(0, n1):
            ...
            while k < n2:
                ...
                k = k + s + 1
            print(f"soft cluster {cl + 1} gene {i} is done!")
    return (upper, lower)
        ...
        for i in range(0, n1):
            for j in range(i + 1, n1):
                ...
                for k in nz_index:
                    ...
                    if iteration:
                        btw_i = (data_full[i, :] <= data_full[i, k] + sdi_0) & (
                                    data_full[i, :] >= data_full[i, k] - sdi_0)
                        btw_j = (data_full[j, :] <= data_full[j, k] + sdj_0) & (
                                    data_full[j, :] >= data_full[j, k] - sdj_0)
                        ...
                        count = 0
                        while ((sdi_0 - sdi_1) ** 2 + (sdj_0 - sdj_1) ** 2 > pow(10, -12)) & (count < maxiter) & (
                                sdi_1 * sdj_1 > 0):
                            sdi_0 = sdi_1
                            sdj_0 = sdj_1
                            btw_i = (data_full[i, :] <= data_full[i, k] + sdi_0) & (
                            ...
                            count = count + 1
                        if count >= maxiter:
                            print(f"Iteration of Cluster {cl+1} at gene {i+1} and gene {j+1} at cell {k+1} have exceed {maxiter}")
                            return
                        sdj = sdj_1
                        sdi = sdi_1
                    else:
                    ...
                    nx = soft_cl[
                        (data_full[i, :] <= data_full[i, k] + sdi) & (data_full[i, :] >= data_full[i, k] - sdi)].sum()
                    ny = soft_cl[
                        (data_full[j, :] <= data_full[j, k] + sdj) & (data_full[j, :] >= data_full[j, k] - sdj)].sum()
                    nxy = soft_cl[
                        (data_full[i, :] <= data_full[i, k] + sdi) & (data_full[i, :] >= data_full[i, k] - sdi) & (
                                    data_full[j, :] <= data_full[j, k] + sdj) & (
                                    data_full[j, :] >= data_full[j, k] - sdj)].sum()
...
def csn_block(data, M=100, g_mtx=None, wd_q=0.1, dev=True, md=1, iteration=False, fuzzy=False, ncore=4):
    ...
    csn_mtx_id = []
    for k in range(0, K):
        csn_mtx_id.append(pd.DataFrame(
            data={'x_id': np.array([], dtype=int), 'y_id': np.array([], dtype=int), 'value': np.array([])}))
    ...
    for i in range(0, n):
        ...
        for j in range(i, n):
            if i == j:
                ...
                csn_id_temp = [sparsetoid(item, i * M, j * M) for item in csn_temp]
                csn_mtx_id = [id_concat(pair) for pair in zip(csn_mtx_id, csn_id_temp)]
                csn_id_temp = [item[item.columns[[1, 0, 2]]].rename(columns={'y_id': 'x_id', 'x_id': 'y_id'}) for item
                               in csn_id_temp]
                csn_mtx_id = [id_concat(pair) for pair in zip(csn_mtx_id, csn_id_temp)]
            print(f"block [{i}, {j}] finished!")
...
def build_ocsn(data, gene_pairs=None, q=0.1):

    n_genes = data.shape[0]

    if gene_pairs is not None:
        g_mtx = np.zeros((n_genes, n_genes))
    ...
    return csn(data, g_mtx=g_mtx, wd_q=q, dev=False, md=1, iteration=False)
...
def compute_nmi(x, y, bins=10):
    def discretize(xx):
        minx, maxx = xx.min(), xx.max()
        if maxx == minx:
            return np.zeros_like(xx, dtype=int)
        return np.floor((xx - minx) / (maxx - minx) * bins).astype(int)
    return normalized_mutual_info_score(discretize(x), discretize(y))



def _build_csn_generic(data, g_mtx, q, weight_func, thresh=5):
    n_genes, n_cells = data.shape
    eps = np.finfo(float).eps

    upper, lower = upperlower(data, boxsize=q)
    ...
    for c in range(n_cells):
        ...
        for m in range(L):
            ...
            in_i = (g1 >= lower[i, c]) & (g1 <= upper[i, c])
            in_j = (g2 >= lower[j, c]) & (g2 <= upper[j, c])
            in_ij = in_i & in_j

            if in_ij.sum() < thresh:
                continue

            idx = np.where(in_ij)[0]
            w = weight_func(g1[idx], g2[idx])
...
def build_pearson(data, gene_pairs=None, q=0.1, thresh=5):
    ...
    if gene_pairs is not None:
        ...
        g_mtx = np.ones((n_genes, n_genes)) - np.tri(n_genes)

    return _build_csn_generic(data, g_mtx, q, compute_pearson, thresh)


def build_spearman(data, gene_pairs=None, q=0.1, thresh=5):
    ...
    return _build_csn_generic(data, g_mtx, q, compute_spearman, thresh)


def build_nmi(data, gene_pairs=None, q=0.1, thresh=5, bins=10):
    ...
    def nmi_weight(x, y):
        return compute_nmi(x, y, bins)

    return _build_csn_generic(data, g_mtx, q, nmi_weight, thresh)
def compute_z(nx, ny, nxy, N, eps=1e-15):

    rho = nxy / N - (nx / N) * (ny / N)
    sigma2 = float(nx) * ny * (N - nx) * (N - ny) / (float(N)**4 * (N - 1))
    if sigma2 < 0:
        sigma2 = 0.0
    return rho / np.sqrt(sigma2 + eps)